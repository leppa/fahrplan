...
//                                date.setSeconds(value.getSeconds());
    ...
    attachedObjects: [
        ...
        SystemProgressDialog {
            ...
            body: modeSelector.selectedIndex == 0 ? qsTr("Searching for journeys...")
                                                  : qsTr("Requesting timetable...")
            confirmButton.label: ""
            cancelButton {
                label: qsTr("Cancel")
            }
        ...
        QtQuick.Connections {
            ...
            onParserTimeTableResult: {
                busyIndicator.cancel();
                if (fahrplan.timetable.childCount([]) > 0) {
                    var page = timetablePageDefinition.createObject();
                    appWindow.push(page);
                }
            }
        }
